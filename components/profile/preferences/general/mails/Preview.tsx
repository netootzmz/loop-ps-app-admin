...
const Preview: FC<{ colors: any; className: string }> = (props) => {
  const { lang } = useSelector<iGlobalState, iUiState>(({ ui }) => ui);
  const { group_description } = useSelector<iGlobalState, iAuthState>(
    ({ auth }) => auth
  );
  ...
  useEffect(() => {
    setCol(props.colors as string);
  }, [props.colors]);

  return (
    <main
      className={`${props.className}`}
      style={{ color: colors.bodyTextType }}
    >
      <Head>
          ...
          <div className="mail-general__top">
            ...
            <picture className="mail-general__picture">
              <Image
                ...
                src={colors.img || "/logo-smart.png"}
                className="mail-general__logo"
                layout="fill"
                blurDataURL="smart-ecommerce-dev.s3.us-east-2.amazonaws.com"
              />
            </picture>
          </div>
