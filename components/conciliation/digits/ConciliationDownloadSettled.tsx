...
export const ConciliationDownloadSettled: FC<{ 
    rowVal: any, 
    bank: string, 
    titles: Partial<any & {}> 
}> = ({ 
    ...
    const downloadFile = (typeReport: string) => {
        ...
        //                 transaction_date: moment(rowVal.fechatxn).format("YYYY-MM-DD"),
    ...
    return(
        <span onClick={()=>downloadFile("SETTLED")} className="conciliationTable__download">
            <SvgWrapper id={svgs.conciliationTableDownload} className="svg svg--small" />
        </span>
    )
