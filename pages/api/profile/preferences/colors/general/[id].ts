...
export default nextConnect<
  NextApiRequest,
  NextApiResponse<iCustomResponse<iGetGeneralMailColors>>
>()
  .use(Cors())
  .use(validateToken)
  .get(async (req, res) => {
    try {
      ...
      const apires = await customApiFetch<
        {},
        iCustomResponse<iGetGeneralMailColors>
      >(
        `${
          url || "http://localhost:8085"
        }/api/v1/smart/e-commerce/configuration/ServicesPortalController/getGeneralMailStyleSettings?clientId=${
          req.query.id
        }`,
        {},
        token
