...
>()
  .use(Cors())
  .use(body("codeVerification", "Code is missing").not().isEmpty())
  .use(body("newPassword", "New password is missing...").not().isEmpty())
  .use(body("newPasswordConfirm", "New password is missing...").not().isEmpty())
  .use(body("userName", "New password is missing...").not().isEmpty())
  .use(validateFields)
  .put(async (req, res) => {
    try {
      ...
      >(
        ...
        }/api/v1/smart/e-commerce/administration/restorePassword/newPassword`,
        req.body
      );
      return res.status(200).json(apires);
    } catch (err) {
      console.log("Error Recover", err);
